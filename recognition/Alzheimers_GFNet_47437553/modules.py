...
class EnhancedMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., use_residual=True):
        super().__init__()
...
class ImprovedGlobalFilter(nn.Module):
    def __init__(self, dim, h=14, w=8, init_scale=0.02):
        super().__init__()
        # Use real+imag last dim for view_as_complex
        self.complex_weight = nn.Parameter(torch.randn(h, w, dim, 2, dtype=torch.float32) * init_scale)
    ...
    def forward(self, x, spatial_size=None):
        ...
        # Attempt to broadcast weight to x_fft's shape
        # weight shape (h, w, dim) -> we expect (a, b//2+1, C) roughly; try simple broadcast
        try:
            x_fft = x_fft * weight
        except Exception:
            # fallback: reshape weight to (1, a, b//2+1, C)
            w_shape = (1, min(a, weight.shape[0]), min(x_fft.shape[2], weight.shape[1]), min(C, weight.shape[2]))
            weight_c = weight
            weight_c = weight_c.reshape((1,)+weight_c.shape)
...
class EnhancedBlock(nn.Module):
    def __init__(self, dim, mlp_ratio=4., drop=0.1, drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, h=14, w=8, use_channel_attention=True, use_layer_scale=True):
        super().__init__()
        ...
        self.mlp = EnhancedMlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
    def forward(self, x):
...
class ImprovedPatchEmbed(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, use_overlap=False):
        super().__init__()
        ...
        if use_overlap:
            num_patches = ((img_size[0] + stride[0] - patch_size[0]) // stride[0] + 1) * ((img_size[1] + stride[1] - patch_size[1]) // stride[1] + 1)
        else:
...
class EnhancedGFNet(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=2, embed_dim=768, depth=12, mlp_ratio=4., drop_rate=0.1, drop_path_rate=0.2, norm_layer=None, use_channel_attention=True, use_layer_scale=True, use_overlap_patch=False):
        super().__init__()
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=1e-6)
        self.patch_embed = ImprovedPatchEmbed(img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, use_overlap=use_overlap_patch)
        ...
        self.blocks = nn.ModuleList([
            EnhancedBlock(dim=embed_dim, mlp_ratio=mlp_ratio, drop=drop_rate, drop_path=dpr[i], norm_layer=norm_layer, h=h, w=w, use_channel_attention=use_channel_attention and (i % 2 == 0), use_layer_scale=use_layer_scale)
...
def create_enhanced_gfnet_small(num_classes=2, img_size=224, **kwargs):
    model = EnhancedGFNet(img_size=img_size, patch_size=16, embed_dim=512, depth=12, mlp_ratio=4, num_classes=num_classes, drop_rate=0.1, drop_path_rate=0.15, use_channel_attention=True, use_layer_scale=True, **kwargs)
    return model

def create_enhanced_gfnet_base(num_classes=2, img_size=224, **kwargs):
    model = EnhancedGFNet(img_size=img_size, patch_size=16, embed_dim=768, depth=14, mlp_ratio=4, num_classes=num_classes, drop_rate=0.08, drop_path_rate=0.18, use_channel_attention=True, use_layer_scale=True, use_overlap_patch=False, **kwargs)
    return model

def create_enhanced_gfnet_large(num_classes=2, img_size=224, **kwargs):
    model = EnhancedGFNet(img_size=img_size, patch_size=16, embed_dim=1024, depth=18, mlp_ratio=4, num_classes=num_classes, drop_rate=0.2, drop_path_rate=0.25, use_channel_attention=True, use_layer_scale=True, use_overlap_patch=True, **kwargs)
    return model
